...
@VaadinView("editarAudio")
@UIComponent
public class editarAudio extends CustomComponent implements View {
	...
	@AutoGenerated
	private Label tf_listasDifusion2;
	@AutoGenerated
	...
	private void init(String idAudio) {

		nuevoAudio = miContenidoService.audiosToEdit(idAudio);

		if (nuevoAudio == null) {
			UI.getCurrent().getNavigator().navigateTo("misAudios");
			return;
		}
		inicializarCampos();

		if (nuevoAudio.getImagen().getBinaryData() != null) {
		...
		nuevoAudio.getListasDifusion().forEach(item -> {
			listasLabel.append(item.getTitulo() + ", ");
			tf_listasDifusion2.setValue(listasLabel.toString());
		});

		StringBuilder categorias = new StringBuilder();
		nuevoAudio.getCategorias().forEach(item -> {
			categorias.append(item.getNombre() + ", ");
	...
	private void iniciarUploadImagen() {
		...
		upload_imagen.addSucceededListener(this::uploadSucceeded);
		upload_imagen.setReceiver(this::imagenReciver);
	...
	public OutputStream imagenReciver(String filename, String mimeType) {
	...
	private void crearAudio(ClickEvent e) {
		...
		try {
			...
			miContenidoService.actualizarAudio(nuevoAudio);

			AudioLearnUI audiolearning = (AudioLearnUI) UI.getCurrent();
			audiolearning.getNavigator().navigateTo("misAudios");

			Notification n = new Notification("Correcto!", "Se ha creado con exito el Audio " + nuevoAudio.getTitulo(),
	...
	private void inicializarCampos() {
		...
		df_fechaCreacion.setResolution(Resolution.SECOND);
	...
	private void style() {
		...
		tf_listasDifusion2.addStyleName("textFieldColor");
	...
	private VerticalLayout buildMainLayout() {
		...
		b_guardar.setCaption("Editar Audio");
	...
	private HorizontalLayout buildHl_cabecera() {
		...
		l_titulo.setValue("Editar Audio");
	...
	private GridLayout buildGridLayout_2() {
		...
		// tf_listasDifusion2
		tf_listasDifusion2 = new Label();
		tf_listasDifusion2.setImmediate(false);
		tf_listasDifusion2.setWidth("100.0%");
		tf_listasDifusion2.setHeight("-1px");
		tf_listasDifusion2.setValue("Label");
		gridLayout_2.addComponent(tf_listasDifusion2, 1, 7);
