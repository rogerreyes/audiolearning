...
public class Reproductor extends CustomComponent implements View {
	...
	private BeanItemContainer<Comentario> BICComentarios;
	...
	public void init(String id) {
		...
		this.b_addComentario.addClickListener(e -> addComentario(audioDB.getId()));

		this.b_calificar.addClickListener(e -> calificarContenido());
		this.b_denunciar.addClickListener(e -> denunciarContenido());
		this.b_favoritos.addClickListener(e -> agregarFavorito() );
		...
		vl_reproductor.setComponentAlignment(this.b_calificar, Alignment.MIDDLE_CENTER);
		...
		if (audioFile != null) {
			...
			BICComentarios = new BeanItemContainer<>(Comentario.class);
			table_1.setContainerDataSource(BICComentarios);

			table_1.addGeneratedColumn("Descripcion", this::construirComentario);
			table_1.addGeneratedColumn("Imagen", this::construirImagen);
			table_1.setVisibleColumns("Imagen", "Descripcion");
			table_1.setColumnWidth("Imagen", 110);

			BICComentarios.removeAllItems();
			if (audioDB.getComentarios() != null) {
				llenarComentarios();
			}
	...
	private void llenarComentarios() {
		BICComentarios.removeAllItems();
		List<Comentario> comentarios = reproductorService.obtenerComentarios(audioDB.getId());
		BICComentarios.addAll(comentarios == null ? new ArrayList<Comentario>() : comentarios);
	}
	...
	private Object construirImagen(Table source, Object itemId, Object columnId) {
		...
		imagen.setSource(new ThemeResource("img/comments-icon.png"));

		return imagen;
	}

	private Object construirComentario(Table source, Object itemId, Object columnId) {
		Comentario comentario = (Comentario) itemId;
		return new LayoutCellDescripcion(comentario.getAutor(), comentario.getComentario(),
				comentario.getFechaComentario(), "A");
	}

	private void addComentario(ObjectId audioID) {

		if (!(ta_Comentario.getValue() == null || ta_Comentario.getValue().trim() == "")) {

			reproductorService.agregarcomentario(audioID, ta_Comentario.getValue());

			Notification n = new Notification("Correcto!", "Registrado el Comentario", Type.HUMANIZED_MESSAGE);
			n.setDelayMsec(3600);
			...
			Notification.show("Aviso!", "Debe escribir un comentario!", Type.WARNING_MESSAGE);
		}
	...
	private void agregarFavorito() {
		try {
			...
			reproductorService.agregarFavorito(audioDB);
			Notification n = new Notification("Favoritos!",
					"Se ha añadio el audio" + audioDB.getTitulo() + " a tu lista de favoritos.", Type.HUMANIZED_MESSAGE);
			n.setDelayMsec(3600);
			n.show(UI.getCurrent().getPage());
	...
	private void style() {
		...
		l_comentarios.addStyleName("mih2");
		...
		b_addComentario.setIcon(VaadinIcons.COMMENTS_O);

		b_calificar.addStyleName(ValoTheme.BUTTON_FRIENDLY);
		b_calificar.setIcon(VaadinIcons.CHECK);
		table_1.addStyleName(ValoTheme.TABLE_NO_VERTICAL_LINES);
		table_1.addStyleName(ValoTheme.TABLE_NO_HEADER);
	...
	private VerticalLayout buildVl_descripcionAudio() {
		...
		// card
		card = buildCard();
		vl_descripcionAudio.addComponent(card);
	...
	private VerticalLayout buildCard() {
		...
		card.setWidth("100.0%");
		card.setHeight("100.0%");
		card.setMargin(true);
		
		// gridLayout_1
		gridLayout_1 = buildGridLayout_1();
		card.addComponent(gridLayout_1);
	...
	private HorizontalLayout buildHorizontalLayout_1() {
		...
		b_addComentario.setCaption("Publicar Comentario");
